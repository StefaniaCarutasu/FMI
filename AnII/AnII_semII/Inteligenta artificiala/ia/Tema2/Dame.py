...
def main():
    ...
    #am jocul computer vs computer
        ...
        while run:
            ...
            if game.turn == computer_color:
                if algorithm == "alphabeta":
                    value, new_board, noOfNodes = AlphaBeta(-500, 500, game.getBoard(), int(depth), computer_color, game, noOfNodes)
                    game.computerMove(new_board)
                else:
                    value, new_board, noOfNodes = MiniMax(game.getBoard(), int(depth), computer_color, game, noOfNodes)
                    game.computerMove(new_board)
            else:
                if algorithm == "alphabeta":
                    value, new_board, noOfNodes = AlphaBeta(-500, 500, game.getBoard(), int(depth), user_color, game, noOfNodes)
                    game.computerMove(new_board)
                else:
                    value, new_board, noOfNodes = MiniMax(game.getBoard(), int(depth), user_color, game, noOfNodes)
                    game.computerMove(new_board)
