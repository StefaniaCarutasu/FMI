...
def writeStats(i, cromozom):
    ...
    f = param_polinom[0] * (x ** 2) + param_polinom[1] * x + param_polinom[2]
...
def nextGen(populatie, pc, pm):
    ...
    for i in populatie:
        c = codificare(D, i)
        fit = param_polinom[0] * (c ** 2) + param_polinom[1] * c + param_polinom[2]
    ...
    u = np.random.rand(len(populatie) - 1)

    for i in range(len(u)):     #parcurgem vectorul de probabilitati
        g.write("u = " + str(u[i]) + " selectam cromozomul ")
        interval = binarySearch(intervale_selectie, u[i])
        if intervale_selectie[interval] != ():       #vedem daca avem interval
            newPopulation.append(populatie[interval])
            g.write(str(interval + 1) + "\n")
    ...
    maximum = param_polinom[0] * (x ** 2) + param_polinom[1] * x + param_polinom[2]
    medie = maximum
    for i in range(1, len(newPopulation)):
        x = codificare(D, newPopulation[i])
        f = int(param_polinom[0]) * (x ** 2) + int(param_polinom[1]) * x + (param_polinom[2])
        if f > maximum:
            maximum = f
        medie += f
    maximum_evolution.append(maximum)
